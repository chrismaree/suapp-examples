...
library RLPEncoder {
    ...
    function encodeUint64(uint64 _val) internal pure returns (bytes memory encoded) {
        if (_val == 0) {
            encoded = new bytes(1);
            encoded[0] = bytes1(uint8(128));
        } else if (_val < 128) {
            encoded = new bytes(1);
            encoded[0] = bytes1(uint8(_val));
        } else {
            encoded = new bytes(9);
            encoded[0] = bytes1(uint8(136)); // 0x88
            for (uint8 i = 8; i > 0; i--) {
                encoded[i] = bytes1(uint8(_val));
                _val >>= 8;
            }
        }
    }
